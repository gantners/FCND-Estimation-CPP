...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
	...
	Quaternion<float> attitude_int = attitude.IntegrateBodyRate(gyro, dtIMU);
	float predictedPitch = attitude_int.Pitch();
	float predictedRoll = attitude_int.Roll();
	ekfState(6) = attitude_int.Yaw();
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
	...
	V3F accel_inert = attitude.Rotate_BtoI(accel);

	//Calculate motion over time for x,y,z and acceleration
	predictedState(0) = curState(0) + curState(3) * dt;
	predictedState(1) = curState(1) + curState(4) * dt;
	predictedState(2) = curState(2) + curState(5) * dt;
	predictedState(3) = curState(3) + accel_inert.x * dt;
	predictedState(4) = curState(4) + accel_inert.y * dt;
	predictedState(5) = curState(5) - 9.81f * dt + accel_inert.z * dt;
